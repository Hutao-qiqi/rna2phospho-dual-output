...
```bash
pip install -r SCP2000/requirements.txt
python SCP2000/predict.py --rna rna.parquet --predicted-protein protein_prediction.parquet --runtime-bundle SCP2000/runtime/scp2000_runtime.npz --output prediction.npz
```
...
`runtime/scp2000_runtime.npz`包含输入中心与尺度、投影矩阵、坐标岭回归、8个坐标修正、位点基底、位点均值、母蛋白映射与系数、全局背景。
