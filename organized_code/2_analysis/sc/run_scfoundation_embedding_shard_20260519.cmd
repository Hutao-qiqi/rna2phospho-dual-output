...
"%PYTHON%" "%CODEDIR%\precompute_scfoundation_embeddings_multidomain.py" --root "%ROOT%" --datasets gse300551_iccite_plex_kinase_2025 signal_seq_gse256403_hela_2024 signal_seq_gse256404_pdo_caf_2024 --batch-size 1 --max-gene-tokens 12000 --num-shards 2 --shard-index %SHARD_INDEX% --skip-existing --output-dir "%OUTDIR%" 1> "%LOGDIR%\embedding_shard%SHARD_INDEX%.log" 2> "%LOGDIR%\embedding_shard%SHARD_INDEX%.err"
set CODE=%ERRORLEVEL%
echo %CODE% > "%LOGDIR%\embedding_shard%SHARD_INDEX%.exit_code.txt"
exit /b %CODE%
