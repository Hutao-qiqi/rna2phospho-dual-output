...
CPTAC_OFFICIAL_RELEASE = ROOT / "02_results/model_validation/20260503_scp682_v4_0_official_phosphosite_release"
...
def predict_cptac(x_ext: pd.DataFrame, out_dir: Path, cancer_label: str, study_id: str, batch_size: int, device: torch.device) -> dict:
    ...
    raise RuntimeError(
        "SCP682 CPTAC/PDC phosphosite output must come from the official frozen release "
        f"{CPTAC_OFFICIAL_RELEASE}. Legacy joint-model weights are not part of SCP682."
    )
