...
class SCP682V4Engine:
    ...
    def predict(self, rna: pd.DataFrame, manifest: pd.DataFrame | None = None) -> dict[str, pd.DataFrame]:
        ...
        v3_total = v3_total.reindex(index=x_ext.index)
        v3_total.index.name = "sample_id"
        v3_total.columns = v3_total.columns.astype(str)

        return {
            "v4_raw": raw,
            "v4_centered": centered,
            "total_protein": v3_total.astype(np.float32),
