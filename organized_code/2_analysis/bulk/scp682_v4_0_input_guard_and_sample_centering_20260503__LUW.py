#!/usr/bin/env python3
"""Input guard and sample median centering for SCP682 v4.0 deployment."""
...
ROOT = Path(os.environ.get("SCP682_PROJECT_ROOT", "/data/lsy/Infinite_Stream/SCP682"))
...
def load_reference_features(path: Path | None) -> list[str]:
    ...
    if path.suffix.lower() == ".parquet":
        ...
        if "gene_symbol" in ref.columns:
            return [str(x) for x in ref["gene_symbol"].dropna().tolist()]
