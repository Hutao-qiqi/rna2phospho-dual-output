"""
模型: SCP682
作用: 从原始实验脚本提取的最小可复现代码片段，文件名 train_scp682_missing_ablation.py
输入: ./data_root 下的训练数据、图先验和冻结基线预测
输出: ./paper_materials_SCP682 或结果目录中的模型、表格、报告
依赖: Python、pandas、numpy、torch、torch_geometric
原始路径: remote_scripts/train_scp682_missing_ablation.py
原始版本: 20260523 结果目录对应脚本
"""

"""Train SCP682 graph-constrained residual ablations.

The base model is described as one integrated predictor:
...
This ablation entry point keeps the model class and loss unchanged, and only
changes graph construction:

    --axis-mode dual | site_only | sample_only
    --edge-mode all | copheemap | copheeksa | kstar | no_copheemap |
                no_copheeksa | no_kstar | rewired_all
"""
...
def identity_edge_index(n):
    return np.asarray([(i, i) for i in range(n)], dtype=np.int64).T


def degree_preserving_rewire(edge_set, n_nodes, seed=20260602, n_swaps_per_edge=10):
    ...
    if len(undirected) < 2:
        return edge_set
    edges = [tuple(x) for x in undirected]
    edge_lookup = set(edges)
    n_attempts = int(max(1, n_swaps_per_edge) * len(edges))
    for _ in range(n_attempts):
        ...
        if e1[0] == e1[1] or e2[0] == e2[1] or e1 == e2:
            continue
        if e1 in edge_lookup or e2 in edge_lookup:
            continue
        edge_lookup.remove(edges[i])
        edge_lookup.remove(edges[j])
        edge_lookup.add(e1)
        edge_lookup.add(e2)
        edges[i] = e1
        edges[j] = e2
...
def build_site_graph(targets, prior_root, max_kinase_neighbors=16, edge_mode="all"):
    ...
    if str(edge_mode).startswith("rewired_"):
        ...
        edge_set = degree_preserving_rewire(edge_set, len(targets), seed=20260602, n_swaps_per_edge=10)
...
def main():
    ...
    ap.add_argument(
        ...
        default="dual",
        help="dual uses both site graph and sample graph; site_only uses site graph plus sample identity; sample_only uses sample graph plus site identity.",
    )
    ap.add_argument(
        "--edge-mode",
        choices=[
