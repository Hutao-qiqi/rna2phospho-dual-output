...
def main():
    ...
    parent = clean_numeric(pd.read_parquet(train / "oof_candidate_parent_only_phosphosite.parquet"))
    ridge = clean_numeric(pd.read_parquet(train / "oof_candidate_ridge_direct_phosphosite.parquet"))
    rna = clean_numeric(pd.read_parquet(train / "oof_candidate_rna_direct_phosphosite.parquet"))
    samples = y_df.index.intersection(parent.index).intersection(ridge.index).intersection(rna.index)
    targets = [c for c in y_df.columns if c in parent.columns and c in ridge.columns and c in rna.columns]
    y_df = y_df.loc[samples, targets]
    baseline_df = (parent.loc[samples, targets] + ridge.loc[samples, targets] + rna.loc[samples, targets]) / 3.0
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == args.epochs:
            ...
            summary = summarize(per, "scp682_exact_scnet_gnn")
            base_per = spearman_cols(y, baseline, mask, targets)
            base_summary = summarize(base_per, "baseline_mean")
            ...
            if row["median_spearman"] > best["median_spearman"]:
                ...
                torch.save({
                    ...
                    "meta": meta,
                    "best": best,
                }, out / "models/scp682_exact_scnet_gnn_best.pt")
                ...
                pd.DataFrame(pred_np, index=samples, columns=targets).to_parquet(out / "predictions/scp682_exact_scnet_gnn_oof_phosphosite_best.parquet")
