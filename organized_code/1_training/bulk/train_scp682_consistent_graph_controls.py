...
def identity_edge_index(n_nodes):
    return np.vstack([np.arange(n_nodes, dtype=np.int64), np.arange(n_nodes, dtype=np.int64)])


def should_use_edge_source(edge_mode, source_name):
    edge_mode = str(edge_mode)
    if edge_mode.startswith("rewired_"):
        edge_mode = edge_mode.replace("rewired_", "", 1)
    if edge_mode == "all":
        return True
    if edge_mode.startswith("no_"):
        return source_name != edge_mode.replace("no_", "", 1)
    return source_name == edge_mode


def degree_preserving_rewire(edge_set, n_nodes, seed=20260603, n_swaps_per_edge=10):
    rng = np.random.default_rng(seed)
    undirected = sorted({tuple(sorted((int(a), int(b)))) for a, b in edge_set if int(a) != int(b)})
    if len(undirected) < 2:
        return edge_set
    edges = [list(e) for e in undirected]
    edge_lookup = {tuple(e) for e in edges}
    n_swap = max(len(edges) * int(n_swaps_per_edge), len(edges))
    for _ in range(n_swap):
        ...
        if rng.random() < 0.5:
            ...
            e1 = tuple(sorted((a, c)))
            e2 = tuple(sorted((b, d)))
        if e1[0] == e1[1] or e2[0] == e2[1] or e1 in edge_lookup or e2 in edge_lookup:
            continue
        old1 = tuple(edges[i])
        old2 = tuple(edges[j])
        edge_lookup.discard(old1)
        edge_lookup.discard(old2)
        edge_lookup.add(e1)
        edge_lookup.add(e2)
        edges[i] = list(e1)
        edges[j] = list(e2)
    rewired = {(i, i) for i in range(n_nodes)}
    for a, b in edge_lookup:
        rewired.add((a, b))
        rewired.add((b, a))
    return rewired
...
def build_site_graph(targets, prior_root, max_kinase_neighbors=16, edge_mode="all"):
    ...
    if should_use_edge_source(edge_mode, "copheemap") and raw_copheemap.exists() and cophee_id_to_gene_site:
    ...
    if should_use_edge_source(edge_mode, "copheeksa") and raw_copheeksa.exists() and cophee_id_to_gene_site:
    ...
    if should_use_edge_source(edge_mode, "kstar") and kstar_long.exists():
    ...
    if should_use_edge_source(edge_mode, "copheemap") and cophee.exists():
    ...
    if should_use_edge_source(edge_mode, "kstar") and kstar.exists():
    ...
    if should_use_edge_source(edge_mode, "copheeksa") and copheeksa.exists():
    ...
    if str(edge_mode).startswith("rewired_"):
        before_directed_edges = len(edge_set)
        edge_set = degree_preserving_rewire(edge_set, len(targets), seed=20260603, n_swaps_per_edge=10)
        source_counts["degree_preserving_rewire_enabled"] = 1
        source_counts["directed_edges_before_rewire"] = before_directed_edges
        source_counts["directed_edges_after_rewire"] = len(edge_set)
...
class SCP682ExactScNETResidual(nn.Module):
    def __init__(self, n_sites, n_samples, shrinkage=0.3, hidden=160, latent=64, inter_dim=192, embd_dim=64, num_layers=2):
        super().__init__()
    ...
    def decode(self, row_embed, col_embed, baseline, mask, site_prior, sample_idx=None):
    ...
    def forward(self, feature_x, col_edge_index, row_edge_index, baseline, mask, site_prior, collect_attention=True, sample_idx=None):
        ...
        pred, delta, graph_delta, residual_delta, attention = self.decode(
            row_embed, col_embed, baseline, mask, site_prior, sample_idx=sample_idx
        )
...
def main():
    ...
    ap.add_argument("--shrinkage", type=float, default=0.3)
    ap.add_argument(
        "--v4-baseline-path",
        default="/data/lsy/Infinite_Stream/02_results/model_validation/20260503_scp682_v4_0_official_phosphosite_release/predictions/SCP682_v4_0_internal_cptac_pdc_phosphosite.parquet",
    )
    ...
    ap.add_argument(
        "--axis-mode",
        choices=["dual", "site_only", "sample_only"],
        default="dual",
        help="dual uses both site graph and sample graph; site_only keeps only the site graph; sample_only keeps only the sample graph.",
    )
    ap.add_argument(
        "--edge-mode",
        choices=["all", "copheemap", "copheeksa", "kstar", "no_copheemap", "no_copheeksa", "no_kstar", "rewired_all"],
        default="all",
        help="site graph edge source selection.",
    )
    args = ap.parse_args()
    ...
    v4_df = clean_numeric(pd.read_parquet(args.v4_baseline_path))
    samples = y_df.index.intersection(v4_df.index)
    targets = [c for c in y_df.columns if c in v4_df.columns]
    y_df = y_df.loc[samples, targets]
    baseline_df = v4_df.loc[samples, targets]
    ...
    edge_index_np, site_prior_np, source_counts = build_site_graph(targets, Path(args.prior_root), edge_mode=args.edge_mode)
    col_edge_np = build_sample_knn_edge_index(baseline, mask, k=args.knn)
    if args.axis_mode == "site_only":
        col_edge_np = identity_edge_index(len(samples))
    elif args.axis_mode == "sample_only":
        edge_index_np = identity_edge_index(len(targets))
        site_prior_np = np.zeros(len(targets), dtype=np.float32)
        source_counts = {"site_graph_disabled_for_sample_only": int(edge_index_np.shape[1])}
    ...
    model = SCP682ExactScNETResidual(
        n_sites=len(targets),
    ...
    meta = {
        ...
        "baseline": "SCP682_v4_0_internal_cptac_pdc_phosphosite",
        "v4_baseline_path": str(args.v4_baseline_path),
        "formula": "phosphosite_hat = v4_hat + lambda * residual_proposal; lambda = 0.3",
        "consistent_loss": "auxiliary residual loss is applied to lambda * residual_proposal, matching the prediction scale",
    }
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start_i in range(0, n_samples, args.batch_size):
            ...
            correction_b = model.shrinkage * delta_b
            graph_correction_b = model.shrinkage * graph_b
            residual_correction_b = model.shrinkage * residual_b
            correction_b_loss = masked_huber(correction_b, r_b, m_b, tensors["site_weight"]) + 0.20 * cosine_loss(correction_b, r_b, m_b)
            batch_obj = pred_b_loss + 0.35 * correction_b_loss + 0.02 * masked_l2(correction_b, m_b, tensors["site_weight"])
            (batch_obj / n_batches).backward(retain_graph=True)
            batch_losses.append(batch_obj.detach())
            pred_losses.append(pred_b_loss.detach())
            delta_losses.append(correction_b_loss.detach())
            attention_values.append(attention_b.detach().mean())
            graph_abs_values.append(graph_correction_b.detach().abs().mean())
            residual_abs_values.append(residual_correction_b.detach().abs().mean())
        ...
        if epoch == 1 or epoch % 10 == 0 or epoch == args.epochs:
            ...
            with torch.no_grad():
                ...
                for start_i in range(0, tensors["baseline"].shape[0], args.batch_size):
                    ...
                    graph_abs_eval.append((model.shrinkage * graph_b).detach().abs().mean().cpu())
                    residual_abs_eval.append((model.shrinkage * residual_b).detach().abs().mean().cpu())
            ...
            summary = summarize(per, "scp682_consistent_shrinkage_loss")
            base_per = spearman_cols(y, baseline, mask, targets)
            base_summary = summarize(base_per, "scp682_v4_baseline")
            ...
            row = {
                ...
                "correction_loss": float(delta_loss.detach().cpu()),
            ...
            if row["median_spearman"] > best["median_spearman"]:
                best = row
                torch.save({
                    ...
                    "meta": meta,
                    "best": best,
                }, out / "models/scp682_consistent_shrinkage_loss_best.pt")
                per.to_csv(out / "tables/per_site_spearman_best.tsv", sep="\t", index=False)
                pd.DataFrame([base_summary, summary]).to_csv(out / "tables/model_summary_best.tsv", sep="\t", index=False)
                pd.DataFrame(pred_np, index=samples, columns=targets).to_parquet(out / "predictions/scp682_consistent_shrinkage_loss_oof_phosphosite_best.parquet")
