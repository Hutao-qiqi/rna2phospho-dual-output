...
训练折按研究、按位点拟合均值和标准差，验证标签只应用这些训练参数；缺少研究元数据或验证研究未知时停止评价。外部预测不读取磷酸化标签，因此不需要用外部研究标签重新拟合尺度。标准化后再执行样本行中位化，最终预测在固定输出词表上再次投影为零中位数。
...
外部样本只连接检查点中的训练参考样本。训练参考真实磷酸化残差仅在通路边权确定后进入分块残差记忆聚合，不能进入查询特征或边权计算。完整约束见 `docs/PATHWAY_SAMPLE_GRAPH_CONTRACT.md`。
