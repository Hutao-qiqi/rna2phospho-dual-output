...
common=(
  ...
  --target-study-residual --study-residual-min-overlap 5
)

for tau in 5 10 20; do
  out_dir=$results/scp682main1_random70_b0_tau${tau}_u64_20260818
  rm -rf "$out_dir"
  mkdir -p "$out_dir/logs"
  echo "[$(date -Is)] running tau=$tau u64"
  "$python" "$code" "${common[@]}" \
    --study-residual-tau "$tau" \
    --output-dir "$out_dir" \
    >"$out_dir/logs/training.stdout.log" \
    2>"$out_dir/logs/training.stderr.log"
  test -f "$out_dir/models/decoder_retrieval_profile_spearman_best.pt"
  touch "$out_dir/SUCCESS"
  echo "[$(date -Is)] tau=$tau done"
done

echo "B0+tau short screen complete"
