...
out=$root/02_results/model_validation/scp682main1_random70_b0_headreset_tau2_u192_20260819
...
"$python" "$code" \
  ...
  --protein-prediction "$input/protein_prediction.parquet" --protein-reliability "$input/protein_reliability.parquet" \
  --protein-provenance "$input/protein_prediction_provenance.tsv" --protein-input-mode crossfit_prediction \
  --phosphosite "$input/phosphosite_logscale_aligned.parquet" --phosphosite-manifest "$manifest" \
  --split-manifest "$input/split_manifest.tsv" --sample-metadata "$input/sample_metadata.tsv" \
  --cophee-prior-bundle "$cophee" --esm2-prior-npz "$esm2" --esm2-fusion residue_residual \
  --device cuda:0 --sample-batch-size 112 --evaluation-batch-size 16 --maximum-kinases-per-site 128 \
  --operator-rank 128 --modality-tokens 32 --phospho-latent-tokens 16 --transformer-layers 2 \
  --site-chunk-size 400 --site-scaling identity_logscale --profile-mse-weight 1 \
  --site-huber-weight 0 --site-pearson-weight 0 --site-rank-weight 0 --profile-pearson-weight 0 \
  --precision bfloat16 --drop-last-training-batch --expected-train-samples 1796 \
  --expected-validation-samples 770 --expected-sealed-samples 0 --early-stopping-metric profile_spearman \
  --validation-interval-updates 16 --epochs 1000 --patience 1000 --seed 20260810 \
  --initial-checkpoint "$stage1/models/decoder_retrieval_profile_spearman_best.pt" \
  --allow-initial-rna-scaler-mismatch --learning-rate 0.00005 --maximum-finetune-updates 192 \
  --target-study-residual --study-residual-tau 2 --study-residual-min-overlap 5 \
  --output-dir "$out" >"$out/logs/training.stdout.log" 2>"$out/logs/training.stderr.log"
test -f "$out/models/decoder_retrieval_profile_spearman_best.pt"
touch "$out/SUCCESS"
