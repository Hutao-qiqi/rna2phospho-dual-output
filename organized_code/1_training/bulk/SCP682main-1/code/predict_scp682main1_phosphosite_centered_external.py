...
EXPERIMENT_ROOT = ROOT / "SCP682main-1"
TRAIN_SCRIPT = EXPERIMENT_ROOT / "code/train_scp682main1_phosphosite_centered.py"
...
DEFAULT_INTERNAL = EXPERIMENT_ROOT / "results/20260720_phosphosite_sample_centered"
DEFAULT_TRAIN_RNA = ROOT / "01_data/multi_omics/processed/pancancer_multi_task_locked_v2/rna_log2_tpm_paired.parquet"
DEFAULT_OUT = EXPERIMENT_ROOT / "results/20260720_phosphosite_centered_external"
...
def run_one_dataset(args, train_mod, ext8, model, ckpt, device, dataset: str, out: Path):
    ...
    train_base_raw_df = train_general_baseline.loc[samples, targets]
    train_base_df = sample_median_center(train_base_raw_df)
    ...
    ext_base_raw_df = ext_general_baseline.loc[:, ext_targets]
    ext_base_df = sample_median_center(ext_base_raw_df)
    ...
    raw_path = out / "predictions" / f"{dataset}_scp682main1_model_output.parquet"
    ...
    true_raw = spec["true_phospho"]
    true_centered = sample_median_center(true_raw)
    summaries = [
        ext8.evaluate(dataset, "phosphosite", "scp682_general_baseline_raw_diagnostic", baseline_raw, true_raw, args.min_site_n, args.min_sample_n, out),
        ext8.evaluate(dataset, "phosphosite", "scp682_general_baseline_sample_centered", baseline_centered, true_centered, args.min_site_n, args.min_sample_n, out),
        ext8.evaluate(dataset, "phosphosite", "scp682main1_graph_residual_sample_centered", pred_centered, true_centered, args.min_site_n, args.min_sample_n, out),
    ]
