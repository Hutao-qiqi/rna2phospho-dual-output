...
def main():
    ...
    y_raw = y_df.to_numpy(np.float32)
    baseline_raw = baseline_df.to_numpy(np.float32)
    target_mask = np.isfinite(y_raw)
    baseline_mask = np.isfinite(baseline_raw)
    mask = target_mask & baseline_mask
    y, target_sample_offset = sample_median_center_array(y_raw, target_mask)
    baseline, baseline_sample_offset = sample_median_center_array(
        baseline_raw, baseline_mask
    )
