...
def site_graph_source_enabled(edge_mode, source):
    mode = str(edge_mode or "all")
    if mode.startswith("rewired_"):
        mode = mode.replace("rewired_", "", 1)
    source = str(source)
    if source == "target_to_bulk_candidate":
        return True
    source_key = {
        "CoPheeMap_onehop": "copheemap",
        "CoPheeKSA_onehop": "copheeksa",
        "KSTAR_onehop": "kstar",
    }.get(source, source.lower())
    if mode == "all":
        return True
    if mode == "copheemap":
        return source_key == "copheemap"
    if mode == "copheeksa":
        return source_key == "copheeksa"
    if mode == "kstar":
        return source_key == "kstar"
    if mode == "no_copheemap":
        return source_key != "copheemap"
    if mode == "no_copheeksa":
        return source_key != "copheeksa"
    if mode == "no_kstar":
        return source_key != "kstar"
    return True
...
def rewire_expanded_graph_preserving_type_degrees(merged, node_table, seed=20260602, n_swaps_per_edge=10):
    ...
    for (a, b), weight in merged.items():
        ...
        if a == b:
            continue
        type_a = str(node_table[a].get("node_type", "node"))
        type_b = str(node_table[b].get("node_type", "node"))
        if (type_a, a) <= (type_b, b):
            ...
            key = (type_b, type_a)
            left, right = b, a
        groups.setdefault(key, {})[(left, right)] = max(float(weight), groups.setdefault(key, {}).get((left, right), 0.0))

    rewired = {}
    for key, edge_map in groups.items():
        edges = [(a, b, w) for (a, b), w in edge_map.items()]
        lookup = {(a, b) for a, b, _ in edges}
        if len(edges) < 2:
            for a, b, w in edges:
                rewired[(a, b)] = max(rewired.get((a, b), 0.0), w)
            continue
        ...
        for _ in range(n_attempts):
            ...
            if e1[0] == e1[1] or e2[0] == e2[1]:
                continue
            if e1 in lookup or e2 in lookup:
                continue
            lookup.remove((a, b))
            lookup.remove((c, d))
            lookup.add(e1)
            lookup.add(e2)
            edges[i] = (e1[0], e1[1], w1)
            edges[j] = (e2[0], e2[1], w2)
        for a, b, w in edges:
            rewired[(a, b)] = max(rewired.get((a, b), 0.0), w)
    ...
    for i in range(len(node_table)):
        out[(i, i)] = 1.0
    for (a, b), weight in rewired.items():
        out[(a, b)] = max(out.get((a, b), 0.0), float(weight))
        out[(b, a)] = max(out.get((b, a), 0.0), float(weight))
    return out


def build_expanded_scnet_site_graph_prior(
    target_rows,
    prior_root,
    candidate_limit=96,
    max_aux_nodes=12000,
    edge_mode="all",
    rewire_seed=20260602,
    ...
    for t, row in enumerate(target_rows):
        ...
        for r in sub.itertuples(index=False):
            ...
            if site_graph_source_enabled(edge_mode, "target_to_bulk_candidate"):
    ...
    if cophee_path.exists() and seed_bulk:
        ...
        for r in cophee.itertuples(index=False):
            ...
            na = get_bulk_node(a)
            nb = get_bulk_node(b)
            if site_graph_source_enabled(edge_mode, "CoPheeMap_onehop"):
        ...
        for _, sub in kin.groupby("kinase"):
            ...
            for seed_idx, seed_score in seeds:
                ...
                for idx, val in vals:
                    ...
                    if site_graph_source_enabled(edge_mode, source):
    ...
    rewired_enabled = str(edge_mode or "all").startswith("rewired_")
    if rewired_enabled:
        before_rewire_edges = len(merged)
        merged = rewire_expanded_graph_preserving_type_degrees(
            merged,
            node_table,
            seed=rewire_seed,
            n_swaps_per_edge=rewire_swaps_per_edge,
        )
    else:
        before_rewire_edges = len(merged)
    ...
    summary = {
        ...
        "degree_preserving_rewire_enabled": bool(rewired_enabled),
        "edges_before_rewire": int(before_rewire_edges),
        "rewire_seed": int(rewire_seed),
        "rewire_swaps_per_edge": int(rewire_swaps_per_edge),
    }
    return {
