"""Frozen SCP2000-sc prediction from RNA counts and row-aligned scFoundation embeddings."""
import argparse
import json
...
import torch
from model import SCP2000sc,normalize_log1p
...
def main():
    ...
    model=SCP2000sc(ns=len(readouts)).to(a.device)
    ck=torch.load(a.release_dir/'SCP2000_sc_U1.pt',map_location='cpu',weights_only=True)
    ...
    (a.output_dir/'inference.json').write_text(json.dumps({'model':'SCP2000-sc','configuration':'U1','domain':a.domain,
