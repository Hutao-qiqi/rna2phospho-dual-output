# SCP2000-sc：正式统一单细胞磷酸化预测模型（U1）

**模型状态：FORMAL CURRENT LOCKED（2026-09-14）。** 正文和图中统一使用名称`SCP2000-sc`；仅在精确说明配置或检查点时使用`U1`。SC11、SC15和F_ALL保留为历史开发模型及消融，不再作为正式推理模型。`SCP682`是本模型此前使用的公开名称，仅用于追溯既有结果。

本发布使用U1配置：GSE300551、icCITE及QuRIE联合训练一个共享模型。输入为4,000个固定基因的表达和3,072维冻结scFoundation表示，经双输入编码、位点条件输出与轻量域校准，预测磷酸化读出。

## 获取模型

[下载完整模型包](https://github.com/Hutao-qiqi/rna2phospho-dual-output/releases/tag/scp2000-sc-u1-v1)。解压后包含`SCP2000_sc_U1.pt`、预处理参数、读出定义、划分清单和可运行代码。
