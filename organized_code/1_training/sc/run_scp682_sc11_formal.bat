...
set OUT=%ROOT%\02_results\single_cell\20260531_scp682_sc11_scp682_main_pathway_token_transfer_v1
set LOG=%OUT%\logs
...
if not exist "%TRANSFER%\tables" (
  echo missing SCP682 main pathway-token transfer package > "%LOG%\fatal.log"
  echo 1 > "%LOG%\formal_exit_code.txt"
  echo %DATE% %TIME% > "%LOG%\formal_finished.txt"
  exit /b 1
)

if not exist "%WARM%\models\scp682_sc7_final.pt" (
  echo missing SCP682-SC warm-start package > "%LOG%\fatal.log"
  echo 1 > "%LOG%\formal_exit_code.txt"
  echo %DATE% %TIME% > "%LOG%\formal_finished.txt"
  exit /b 1
)
